...
This project contains a working example scene `Example/Example.unity` which gives an overview of the whole system. If you are setting up a VRChat avatar, please read [VRCA addon](Addon/VRCA) instruction.
...
The `rootBone` property of this SkinnedMeshRenderer determines the origin of recording space. It's set to be the recorder by default but it can be any Transform. You may add an emote/gesture toggle to keep it fixed in world space, so that your locomotion is recorded.
...
* FPS value: 60. Lower fps is very visible in VR

Output/Streaming:
* Bitrate: 400 ~ 800 Kbps. Scale the bitrate properly if your resolution or FPS is larger than the minimum recommended settings above.
